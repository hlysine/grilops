...
export default function quadTree<Name extends string>({
  ...
  const { fastAnd } = fastZ3({ context, lowLevel });
  /**
   ...
   */
  return class ExpressionQuadTree<ExprKey extends string | number | symbol> {
    ...
    private _tl: ExpressionQuadTree<ExprKey> | undefined;
    private _tr: ExpressionQuadTree<ExprKey> | undefined;
    private _bl: ExpressionQuadTree<ExprKey> | undefined;
    private _br: ExpressionQuadTree<ExprKey> | undefined;
    private _quads: ExpressionQuadTree<ExprKey>[] = [];
      ...
      if (!this._point) {
        ...
        const make = (cond: (p: Point) => boolean) => {
          ...
          if (quadPoints.length > 0) {
            return new ExpressionQuadTree(quadPoints, exprFuncs);
          }
          return undefined;
        };
    ...
    public getOtherPointsExpr(
      ...
      if (coveredPoints.length > 0) {
        ...
        return fastAnd(...terms);
      }

      let expr = this._exprs.get(key);
      if (!expr) {
        expr = fastAnd(...this.getExprs(key));
        this._exprs.set(key, expr);
      }
      return expr;
    }
